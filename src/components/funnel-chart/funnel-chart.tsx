...
function hSegmentPath(
  ...
  const my = H / 2;
  const h0 = normStart * H * 0.5 * layerScale;
  const h1 = normEnd * H * 0.5 * layerScale;
...
function vSegmentPath(
  ...
  const mx = W / 2;
  const w0 = normStart * W * 0.5 * layerScale;
  const w1 = normEnd * W * 0.5 * layerScale;
...
function SegmentLabel({
  ...
  const valueEl = showValues && (
    <span className="whitespace-nowrap font-semibold text-foreground text-base sm:text-xl">
      {display}
    </span>
  );
  ...
  const labelEl = showLabels && (
    <span className="whitespace-nowrap font-medium text-muted-foreground text-xs sm:text-sm">
      {stage.label}
    </span>
  );
